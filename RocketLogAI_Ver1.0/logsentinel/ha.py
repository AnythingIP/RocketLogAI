"""
Deep, privacy-first Home Assistant integration for LogSentinel.
...
- Trigger rich alerting on verified major threats:
    ...
    * update/create sensors (logsentinel_open_threats, logsentinel_last_threat)
...
class HomeAssistantClient:
    ...
    def create_persistent_notification(self, message: str, title: str = "LogSentinel Alert", notification_id: str | None = None) -> bool:
        try:
    ...
    def update_sensor(self, entity_id: str, state: str, attributes: dict[str, Any]) -> bool:
        """Create or update a sensor.* entity from LogSentinel."""
        try:
            payload = {
                "state": str(state),
    ...
    def trigger_major_threat_alert(
        ...
        message = (
            f"🚨 **{severity}** threat verified by LogSentinel\n\n"
            f"**{summary}**\n\n"
            f"Host: {hostname}\n"
        ...
        results["notification"] = self.create_persistent_notification(
            message,
            title=f"LogSentinel • {severity} Alert",
            ...
            for svc in notify_services:
                if "." in svc:
                    ...
                    ok = self.call_service(
                        domain,
                        service,
                        {"message": message, "title": f"LogSentinel {severity} Alert"},
                    )
                    if ok:
...
def get_ha_client(cfg: Any | None = None) -> HomeAssistantClient | None:
    """
    Factory that respects LogSentinel Config.
