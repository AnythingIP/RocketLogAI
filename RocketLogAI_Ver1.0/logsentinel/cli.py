...
def run(ctx, foreground: bool, web: bool, web_host: str, web_port: int, web_user: str | None, web_password: str | None):
    ...
    async def main_loop():
        ...
        # Start heartbeat / deep service monitoring (if enabled)
        heartbeat_task = None
        if cfg.heartbeats.enabled and cfg.heartbeats.monitors:
            ...
            async def _hb_loop():
                while True:
                    ...
                    await asyncio.sleep(30)  # check frequently, individual monitors have their own intervals
            heartbeat_task = asyncio.create_task(_hb_loop())
            logger.info("Heartbeat monitoring enabled for %d services", len(cfg.heartbeats.monitors))
