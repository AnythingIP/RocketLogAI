...
class SyslogServer:
    ...
    async def start_tls(self, certfile: str, keyfile: str, tls_port: int = 6514) -> None:
        """Start an RFC 5425 TLS syslog listener (secure syslog). Uses the same callback pipeline."""
        self._running = True
        try:
            ...
            logger.info(f"Listening for SECURE TLS syslog on {self.host}:{tls_port} (RFC 5425 style)")
        except Exception as e:
            logger.error(f"Failed to start TLS syslog listener: {e}")
    ...
    async def stop(self) -> None:
        ...
        logger.info("Syslog server stopped (including TLS if active)")
    ...
    async def _handle_tls_client(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter) -> None:
        """Handle one TLS syslog client (secure channel)."""
        peer = writer.get_extra_info("peername")
        try:
            while self._running:
                try:
                ...
                raw = data.decode("utf-8", errors="replace").strip()
                if raw:
                    await self._process_message(raw, source=f"tls:{peer[0]}" if peer else "tls")
        finally:
            writer.close()
            try:
                await writer.wait_closed()
