...
# LocalLLM now accessed via get_llm_client factory (PR1)
...
class Analyzer:
    ...
    async def analyze_recent(self) -> dict[str, Any]:
        ...
        for rec in recent:
            ...
            # Noise suppression: if a *custom* low-severity rule matched (e.g. our HA addon UPS spam patterns),
            # cap the rule score low so it doesn't force LLM or high-severity threats.
            # (The log is still stored and visible in /logs + AI assistant.)
            low_custom = any(
                (m.get("rule_id") == "custom" and m.get("severity") == "low")
                for m in rec["_rule_matches"]
            )
            if low_custom:
                rec["_rule_score"] = min(score, 1.5)

            effective_score = rec["_rule_score"]
            if effective_score >= 6.0 or rec.get("severity_code", 0) >= 4:
        ...
        llm_candidates = [r for r in llm_candidates if not _is_low_noise(r)]
            ...
            if ip:
                ...
                if geo_data:
                    t.update({
                        ...
                        "geo_source": geo_data.get("source") or (geo_data.get("sources")[0] if geo_data.get("sources") else None),
                    })
                    # Also cache it
